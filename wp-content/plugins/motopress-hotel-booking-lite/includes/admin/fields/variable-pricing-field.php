...
class VariablePricingField extends InputField {
					...
					for ( $i = 0, $count = count( $periods ); $i < $count; $i++ ) {
						$result .= '<td data-period-index="' . esc_attr( $i ) . '">';
							if ( $periods[$i] == 1 ) {
								...
								$result .= '<br /><span class="dashicons dashicons-trash mphb-pricing-action mphb-pricing-remove-period" title="' . __( 'Remove', 'motopress-hotel-booking' ) . '"></span>';
							}
						$result .= '</td>';
					}

					$result .= '<td><span class="dashicons dashicons-plus mphb-pricing-action mphb-pricing-add-period" title="' . __( 'Add length of stay', 'motopress-hotel-booking' ) . '"></span></td>';
				$result .= '</tr>';
	...
	protected function generateTemplate(){
		...
		$result .= '<tr class="mphb-pricing-variation-template mphb-hide" data-index="' . esc_attr( $index ) . '">';
			...
			$result .= '<td>';
				$result .= '<span class="dashicons dashicons-trash mphb-pricing-action mphb-pricing-remove-variation" title="' . __( 'Remove variation', 'motopress-hotel-booking' ) . '"></span>';
			$result .= '</td>';
		$result .= '</tr>';
	...
	protected function renderAdults( $name, $value, $atts = '', $class = '' ){
		...
		$max = MPHB()->settings()->main()->getSearchMaxAdults();

		return '<input type="number" name="' . esc_attr( $this->getName() . $name ) . '" class="' . esc_attr( 'small-text ' . $class ) . '" value="' . esc_attr( $value ) . '" min="' . esc_attr( $min ) . '" max="' . esc_attr( $max ) . '" step="1" ' . $atts . ' />';
	}

	protected function renderChildren( $name, $value, $atts = '', $class = '' ){
		$min = MPHB()->settings()->main()->getMinChildren();
		$max = MPHB()->settings()->main()->getSearchMaxChildren();
